...
const getFiles = async (url = 'https://www.sba.gov/api/content/40961.json') => {

  let files = [];
  console.log('getting files')
  const res = await fetch(url);
  const body = await res.text();
  const bodyPasred = JSON.parse(body || '{}');
  files = _.get(bodyPasred, 'files', []).filter(f => f.version.toLowerCase().includes('year')).map(f => f.fileUrl);
  return files;
