  ...
  with disaster_declarations_summary_grouped_for_merge as (
        ...
        with t as (
          ...
          rank()
            OVER (
                ...
                ORDER BY (fema_incident_begin_date - begin_date_time)
                ) as rank,
          FIRST_VALUE(disaster_number)
            OVER (
                PARTITION BY event_id
                ORDER BY (fema_incident_begin_date - begin_date_time)
                ) as disaster_number,
          ...
          JOIN disaster_declarations_summary_grouped_for_merge d
            ON substring (sw.geoid, 1, 5) = d.geoid
    ...
    swd as (
        SELECT
          ...
          coalesce (sum(deaths_direct):: float, 0) as deaths_direct,
          coalesce (sum(deaths_indirect):: float, 0) deaths_indirect,
          coalesce (sum(injuries_direct):: float, 0) injuries_direct,
          coalesce (sum(injuries_indirect):: float, 0) injuries_indirect
        FROM ${nceie_schema}.${nceie_table} sw
