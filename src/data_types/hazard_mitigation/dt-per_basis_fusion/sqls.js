export const per_basis_swd = (table_name, view_id, fusion_schema, fusion_table, startYear, endYear,) => `
    with buildings as (select 'buildings'            ctype,
                              coalesce(event_id::text, disaster_number),
         ...
         crop as (select 'crop'                 ctype,
                  ...
                  WHERE extract(YEAR from coalesce(swd_begin_date, fema_incident_begin_date)) between ${startYear} and ${endYear}
                    ...
                    AND nri_category is not null
                  GROUP BY 1, 2, 3, 4),
         population as (select 'population'           ctype,
                               coalesce(event_id::text, disaster_number),
                               substring(geoid, 1, 5) geoid,
                               nri_category           nri_category,
                               min(coalesce(swd_begin_date, fema_incident_begin_date))::date      swd_begin_date,
                               max(coalesce(swd_end_date, fema_incident_end_date))::date        swd_end_date,
                               sum(swd_population_damage) * 11600000 damage
                        FROM ${fusion_schema}.${fusion_table}
                        WHERE extract(YEAR from coalesce(swd_begin_date, fema_incident_begin_date)) between ${startYear} and ${endYear}
                          AND geoid is not null
                          AND nri_category is not null
    ...
    order by id
        ...
        day_expansion as (
        SELECT ctype, generate_series(swd_begin_date:: date, LEAST(
            swd_end_date:: date, CASE WHEN nri_category = 'drought' THEN swd_begin_date:: date + INTERVAL '365 days' ELSE swd_begin_date:: date + INTERVAL '31 days' END
            ), '1 day':: interval):: date event_day_date, nri_category, geoid, sum (damage:: double precision /LEAST(
            swd_end_date:: date - swd_begin_date:: date + 1, CASE WHEN nri_category = 'drought' THEN 365 ELSE 31 END)) damage
        FROM alldata
    ...
    INTO ${fusion_schema}.${table_name}_${view_id}
    FROM final;
`;
...
with cpi as (
  SELECT unnest(array[1996, 1997, 1998, 1999, 2000, 2001, 2002, 2003, 2004, 2005, 2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022]) as YEAR,
    unnest(array[154.4, 159.1, 161.6, 164.3, 168.8, 175.1, 177.1, 181.7, 185.2, 190.7, 198.3, 202.4, 211.1, 211.143, 216.687, 220.223, 226.655, 230.28, 233.916, 233.707, 236.916, 242.839, 247.867, 251.712, 257.971, 261.582, 281.148]) as JAN,
    unnest(array[154.9, 159.6, 161.9, 164.5, 169.8, 175.8, 177.8, 183.1, 186.2, 191.8, 198.7, 203.5, 211.7, 212.193, 216.741, 221.309, 227.663, 232.166, 234.781, 234.722, 237.111, 243.603, 248.991, 252.776, 258.678, 263.014, 283.716]) as FEB,
    unnest(array[155.7, 160, 162.2, 165, 171.2, 176.2, 178.8, 184.2, 187.4, 193.3, 199.8, 205.4, 213.5, 212.709, 217.631, 223.467, 229.392, 232.773, 236.293, 236.119, 238.132, 243.801, 249.554, 254.202, 258.115, 264.877, 287.504]) as Mar,
    unnest(array[156.3, 160.2, 162.5, 166.2, 171.3, 176.9, 179.8, 183.8, 188, 194.6, 201.5, 206.7, 214.8, 213.24, 218.009, 224.906, 230.085, 232.531, 237.072, 236.599, 239.261, 244.524, 250.546, 255.548, 256.389, 267.054, 289.109]) as Apr,
    unnest(array[156.6, 160.1, 162.8, 166.2, 171.5, 177.7, 179.8, 183.5, 189.1, 194.4, 202.5, 207.9, 216.6, 213.856, 218.178, 225.964, 229.815, 232.945, 237.9, 237.805, 240.236, 244.733, 251.588, 256.092, 256.394, 269.195, 292.296]) as May,
    unnest(array[156.7, 160.3, 163, 166.2, 172.4, 178, 179.9, 183.7, 189.7, 194.5, 202.9, 208.4, 218.8, 215.693, 217.965, 225.722, 229.478, 233.504, 238.343, 238.638, 241.038, 244.955, 251.989, 256.143, 257.797, 271.696, 296.311]) as Jun,
    unnest(array[157, 160.5, 163.2, 166.7, 172.8, 177.5, 180.1, 183.9, 189.4, 195.4, 203.5, 208.3, 219.964, 215.351, 218.011, 225.922, 229.104, 233.596, 238.25, 238.654, 240.647, 244.786, 252.006, 256.571, 259.101, 273.003, 296.276]) as Jul,
    unnest(array[157.3, 160.8, 163.4, 167.1, 172.8, 177.5, 180.7, 184.6, 189.5, 196.4, 203.9, 207.9, 219.086, 215.834, 218.312, 226.545, 230.379, 233.877, 237.852, 238.316, 240.853, 245.519, 252.146, 256.558, 259.918, 273.567, 296.171]) as Aug,
    unnest(array[157.8, 161.2, 163.6, 167.9, 173.7, 178.3, 181, 185.2, 189.9, 198.8, 202.9, 208.5, 218.783, 215.969, 218.439, 226.889, 231.407, 234.149, 238.031, 237.945, 241.428, 246.819, 252.439, 256.759, 260.28, 274.310, 296.808]) as Sep,
    unnest(array[158.3, 161.6, 164, 168.2, 174, 177.7, 181.3, 185, 190.9, 199.2, 201.8, 208.9, 216.573, 216.177, 218.711, 226.421, 231.317, 233.546, 237.433, 237.838, 241.729, 246.663, 252.885, 257.346, 260.388, 276.589, 298.012]) as Oct,
    unnest(array[158.6, 161.5, 164, 168.3, 174.1, 177.4, 181.3, 184.5, 191, 197.6, 201.5, 210.2, 212.425, 216.33, 218.803, 226.23, 230.221, 233.069, 236.151, 237.336, 241.353, 246.669, 252.038, 257.208, 260.229, 277.948, 297.711]) as Nov,
    unnest(array[158.6, 161.3, 163.9, 168.3, 174, 176.7, 180.9, 184.3, 190.3, 196.8, 201.8, 210, 210.228, 215.949, 219.179, 225.672, 229.601, 233.049, 234.812, 236.525, 241.432, 246.524, 251.233, 256.974, 260.474, 278.802, 296.797]) as Dec,
    unnest(array[156.9, 160.5, 163, 166.6, 172.2, 177.1, 179.9, 184, 188.9, 195.3, 201.6, 207.3, 215.303, 214.537, 218.056, 224.939, 229.594, 232.957, 236.736, 237.017, 240.007, 245.12, 251.107, 255.657, 258.811, 270.970, 292.655]) as AVG
  ),
    cpi22 as (
    select *
    from cpi
    ...
    select
        ...
        geoid,
        nri_category,
        LEAST(extract(YEAR from event_day_date), 2022) as year, -- adjusting to 2022 dollars
        ...
        CASE WHEN extract(MONTH from event_day_date) = 1
             ...
             WHEN extract(MONTH from event_day_date) = 12
                 THEN dec
            END AS month_2022_cpi
    from ${fusion_schema}.${table_name}_${view_id}, cpi22
