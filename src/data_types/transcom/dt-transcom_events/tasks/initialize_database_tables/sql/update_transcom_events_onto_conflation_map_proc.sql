...
DROP PROCEDURE IF EXISTS _transcom_admin.update_transcom_events_onto_conflation_map();

CREATE OR REPLACE PROCEDURE _transcom_admin.update_transcom_events_onto_conflation_map()
  LANGUAGE plpgsql
  AS $$
    DECLARE
    ...
    BEGIN
      ...
      EXECUTE FORMAT ('
          ...
          CREATE TABLE IF NOT EXISTS _transcom_admin.%I (
            LIKE transcom.transcom_events_onto_conflation_map INCLUDING ALL
          ) WITH (fillfactor=100);
        ',
        table_name
      ) ;
