...
import { NpmrdsDatabaseSchemas } from "../../../src/data_types/npmrds/domain";
...
export const leaf_table_re = /^tmc_identification_[a-z]{2}_\d{4}_v\d{8}t\d{6}$/;
...
export async function getTmcIdentificationInheritanceTreesByYear(): Promise<InheritanceTreesByYear> {
  ...
  const parent_table_re = /^tmc_identification_\d{4}/;

  const root_tables_by_year: Record<string, InheritanceTree> = {};

  for (const row of rows) {
    const { table_schema, table_name, depth } = row;

    if (depth > 2) {
      throw new Error(`INVARIANT BROKEN: depth of ${depth} is greater than 2`);
    }

    const is_leaf = depth === 2;

    const re = is_leaf ? leaf_table_re : parent_table_re;
    ...
    const year = table_name.split("_")[is_leaf ? 3 : 2];
...
export function parseLeafTableInfo(table_info: TableInfo) {
  ...
  if (table_schema !== NpmrdsDatabaseSchemas.NpmrdsTmcIdentificationImports) {
    throw new Error(
      `table_schema ${table_schema} !== ${NpmrdsDatabaseSchemas.NpmrdsTmcIdentificationImports}.`
    );
  }
  ...
  const [, , state, year, version] = table_name.split("_");
  ...
  return { state, year, start_date, end_date, version };
}
...
export function getRootTable(inheritance_tree: InheritanceTree): {
  ...
  console.log(JSON.stringify({ inheritance_tree }, null, 4));
